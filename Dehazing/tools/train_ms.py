...
from dehazing.utils.schedulers import get_cosine_schedule_with_warmup
from dehazing.losses import CharbonnierLoss, LossNetwork, ms_ssim, TVLoss
from dehazing.metrics import lpips, torch_psnr, torch_ssim
from dehazing.utils import checkpoint
...
def train(epoch, train_loader):
    ...
    for i in train_tqdm:
        ...
        loss = 0
        for j, scale in enumerate(cfg.DATASETS.TRAIN.SCALES):
            images_batch = F.interpolate(images_batch, scale, mode='bilinear', align_corners=True)
            labels_batch = F.interpolate(labels_batch, scale, mode='bilinear', align_corners=True)

            model_out, log_dict = model(images_batch)
            ...
            loss += loss_scale
            loss_dict[f"loss_scale{j}"] = f"{loss.item():.4f}"
...
def eval(epoch):
    ...
    logger.info('===>HR: testing psnr = {:.2f}, ssim = {:.3f}, perceptual = {:.3f}, time: {:.2f}'.format(psnr_hr_mean, ssim_hr_mean, perceptual_hr_mean,(end - begin)))
...
def test(epoch):
    ...
    pred_gt = F.interpolate(pred_gt, size=(128, 192), mode='bilinear')
    grid = make_grid(pred_gt, nrow=5)
...
def main():
    ...
    crop_phase = 0
    cnt = 0
    dataset = DehazingTrainDataset(cfg, crop_size=cfg.DATASETS.TRAIN.CROP_SIZE[0])
    for epoch in range(start_epoch, cfg.OPTIMIZER.MAX_EPOCH):
        ...
        psnr_lr_all, ssim_lr_all, perceptual_lr_all, psnr_lr_mean, ssim_lr_mean, perceptual_lr_mean, \
        psnr_hr_all, ssim_hr_all, perceptual_hr_all, psnr_hr_mean, ssim_hr_mean, perceptual_hr_mean = eval(epoch)
        _ = test(epoch)
        ...
        writer.add_scalar('PSNR_HR/val', psnr_hr_mean, epoch)
        writer.add_scalar('SSIM_HR/val', ssim_hr_mean, epoch)
        writer.add_scalar('Perceptual_HR/val', perceptual_hr_mean, epoch)
